...
/*******************************************************************************
* Function Name: LED_Driver_SpiCyBtldrCommStart
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: LED_Driver_SpiCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: LED_Driver_SpiCyBtldrCommReset
****************************************************************************//**
*
...
void LED_Driver_SpiCyBtldrCommReset(void)
{
    ...
    LED_Driver_SpiUartClearRxBuffer();
    LED_Driver_SpiUartClearTxBuffer();
}


/*******************************************************************************
* Function Name: LED_Driver_SpiCyBtldrCommRead
****************************************************************************//**
*
...
cystatus LED_Driver_SpiCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        {
            /* Check packet start */
            if (0u != LED_Driver_SpiUartGetRxBufferSize())
            {
                /* Wait for packet end */
                do
                {
                    byteCount = LED_Driver_SpiUartGetRxBufferSize();
                    CyDelayUs(LED_Driver_SPI_BYTE_TO_BYTE);
                }
                while (byteCount != LED_Driver_SpiUartGetRxBufferSize());

                /* Disable data reception into RX FIFO */
                LED_Driver_RX_FIFO_CTRL_REG |= LED_Driver_RX_FIFO_CTRL_FREEZE;

                byteCount = LED_Driver_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) LED_Driver_SpiUartReadRxData();
        }
    }
...
/*******************************************************************************
* Function Name: LED_Driver_SpiCyBtldrCommWrite
****************************************************************************//**
*
*  Allows the caller to read data from the bootloader host (the host writes the
*  data). The function handles polling to allow a block of data to be completely
...
cystatus LED_Driver_SpiCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        LED_Driver_SpiUartPutArray(pData, (uint32) size);

        /* Wait with timeout 1mS for packet end */
        do
        {
            /* Check for packet end */
            if ((0u == LED_Driver_SpiUartGetTxBufferSize()) &&
                (0u == (LED_Driver_SPI_STATUS_REG & LED_Driver_SPI_STATUS_BUS_BUSY)) &&
                (0u == (LED_Driver_TX_FIFO_STATUS_REG & LED_Driver_TX_FIFO_SR_VALID)))
            {
                *count = size;
                status = CYRET_SUCCESS;
        ...
        LED_Driver_SpiUartClearRxBuffer();
        LED_Driver_RX_FIFO_CTRL_REG &= (uint32) ~LED_Driver_RX_FIFO_CTRL_FREEZE;
    }
