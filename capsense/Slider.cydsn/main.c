...
static struct_animation_t    vec_call_back_anim  [ANIM_UNKNOWN]       ={{ANIM_LEFT_RIGHT        , animate_left_right     , "left-right"                        }, 
                                                                        ...
                                                                        {ANIM_FIX_WHITE         , animate_fix_white      , "4 fix white LEDs at full intensity"}};                                                                   
        ...
        case CMD_TRACKER:
        {
            ...
            switch (tracker_cmd_id = get_tracker_cmd_id_from_cmd_string(param_str))
            {
                ...
                case CMD_TRACKER_SET_BUTTON:
                {
                    ...
                    param_str++;
                    number_of_buttons = atoi(param_str);
                    break;
                }// CMD_TRACKER_SET_BUTTON
...
{
#if 1    
    uint16 led_start = (button * button_width) + 1;
    uint16 led_stop  = led_start + button_width;
    uint16 i;
    
    set_led(led_start, intensity / 2);
    set_led(led_stop , intensity / 2);
    ...
    switch (button)
    {
        case 0: set_led(1, intensity / 2);
                set_led(2, intensity    );
                set_led(3, intensity / 2);
                break;
        case 1: set_led(3, intensity / 2);
                set_led(4, intensity    );
                set_led(5, intensity / 2);
                break;
        case 2: set_led(5, intensity / 2);
                set_led(6, intensity    );
                set_led(7, intensity / 2);
                break;
        case 3: set_led(7, intensity / 2);
                set_led(8, intensity    );
                set_led(9, intensity / 2);
                break;
        case 4: set_led(9 , intensity / 2);
                set_led(10, intensity    );
                set_led(11, intensity / 2);
                break;
        case 5 : 
        default:
                set_led(12, intensity / 2);
                set_led(13, intensity    );
                set_led(14, intensity / 2);
                break; 
    }
...
{
    int    new_led_pos  = 0;
    uint16 led_buckets  = SLIDER_RESOLUTION    / number_of_buttons;
    uint16 button_width = NUMBER_OF_WHITE_LEDS / number_of_buttons;
    ...
    if (cap_cur_pos != cap_old_pos)
    {
        ...
        if ((cap_cur_pos != 0u) && (new_led_pos != led_pos) && (new_led_pos >= 0) && (new_led_pos <= NUMBER_OF_WHITE_LEDS - 1))
        {
            if (tracker_enable)
            {
                if (button_width > 1)
                {
                    display_button(led_pos    , 0x00         , button_width);
                    display_button(new_led_pos, MAX_INTENSITY, button_width);
                }
                else
                {
