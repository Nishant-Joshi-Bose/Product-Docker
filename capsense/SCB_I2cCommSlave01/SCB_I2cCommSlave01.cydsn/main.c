...
int main()
{
    ...
    ***************************************************************************/
    for (;;)
    {
        ...
        // Fake that we have data to read (every 100th cycle)
        if (i >= 1000000) {
            i = 0;
            memset(i2cReadBuffer, 0xB0, BUFFER_SIZE);
            i2cReadBuffer[1] = 0x0B;
            cap_int_Write(1u);
...
uint8 ExecuteCommand(uint32 cmd)
{
    ...
    LED_RED_Write  (cmd & 0x01);
    LED_GREEN_Write((cmd & 0x02) >> 1);
    LED_BLUE_Write ((cmd & 0x04) >> 2);
