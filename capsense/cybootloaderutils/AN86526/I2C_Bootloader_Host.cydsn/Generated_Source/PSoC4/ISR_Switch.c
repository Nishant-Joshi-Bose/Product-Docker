...
{
    CyRamVectors[CYINT_IRQ_BASE + ISR_Switch__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + ISR_Switch__INTC_NUMBER];
}
...
void ISR_Switch_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((ISR_Switch__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *ISR_Switch_INTC_PRIOR = (*ISR_Switch_INTC_PRIOR & (uint32)(~ISR_Switch__INTC_PRIOR_MASK)) |
...
uint8 ISR_Switch_GetPriority(void)
{
    ...
    priority = (*ISR_Switch_INTC_PRIOR & ISR_Switch__INTC_PRIOR_MASK) >> priorityOffset;
