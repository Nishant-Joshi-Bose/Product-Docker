...
uint16 display_finger_tracker      (uint16 last_led_pos, uint32 centroid_pos);
uint16 get_button_pos              (uint16 led_pos);
...
int main()
{
    ...
    for(;;)
    {
        if ((CapSense_IsBusy() != CapSense_NOT_BUSY) && cap_sense_is_untouched && get_is_anim_on_button_up_enable())
        {
        ...
        if (cap_sense_is_untouched)
        {
            if (! cap_sense_was_untouched)
            {
                ...
                send_button_telemetry(get_button_pos(led_pos), "up", finger_direction, cap_cur_pos, cap_old_pos);
        ...
        else
        {
            if (cap_sense_was_untouched)
            {
                tmp = get_button_pos(get_led_pos(cap_cur_pos));
                reset_timer();
            ...
            if ((cap_cur_pos != CapSense_SLIDER_NO_TOUCH) && (cap_cur_pos != cap_old_pos))
            {
                ...
                new_led_pos = display_finger_tracker (led_pos, cap_cur_pos);
                if (new_led_pos != led_pos)
                {
                    led_pos = new_led_pos;
                    send_button_telemetry(get_button_pos(led_pos), "position", finger_direction, cap_cur_pos, cap_old_pos);
                }
            }
...
uint16 display_finger_tracker(uint16 last_led_pos, uint32 centroid_pos)
{
    ...
    uint16 new_led_pos  = get_led_pos(centroid_pos);

    if (get_is_tracker_enable())
    {
        if (button_width > 1)
        {
            display_button(get_button_pos(last_led_pos), 0x00         , button_width);
            display_button(get_button_pos(new_led_pos ), MAX_INTENSITY, button_width);
        }
        else
        {
            set_led(last_led_pos, 0x00         );
...
{   
    ...
    return min(number_of_buttons, max(1, led_pos / (NUMBER_OF_WHITE_LEDS / number_of_buttons)));
}// get_button_pos
