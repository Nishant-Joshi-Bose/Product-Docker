...
/* keep a multiple of 2 blocks since that is the amount read at a time. */
#define MAX_EDID_BLOCKS 6
...
#define HDCP_AUTHENTICATED_INT_EN BIT(1)

/* 0x95 interrupts */
#define HDCP_ERROR_INT_BIT       BIT(7)
#define BKSV_FLAG_INT_BIT        BIT(6)
...
#define HDCP_INTERRUPTS          (HDCP_ERROR_INT_BIT | \
		                          BKSV_FLAG_INT_BIT)
...
#define HDCP_HDMI_DVI_REG        (0xAF)
#define HDMI_DVI_SEL_BIT         BIT(1)
#define HDCP_DESIRED_BIT         BIT(7)

#define HDCP_ERR_STATE_REG       (0xC8)
#define HDCP_CONTROLLER_ERROR_BITS (BIT(7) | BIT(6) | BIT(5) | BIT(4))
#define HDCP_CONTROLLER_STATE_BITS (BIT(3) | BIT(2) | BIT(1) | BIT(0))

#define HDCP_ENC_ON_REG            (0xB8)
#define HDCP_ENC_ON_BIT            BIT(6)

#define SPDIF_REG                  (0x0B)
#define SPDIF_ENABLE_BIT           BIT(7)
#define SPDIF_ENABLED_VAL          (0x8E)
#define SPDIF_DISABLED_VAL         (0x0E)
...
#define HDCP_CHECK_DELAY_MS (2000)
...
struct adv7533 {
	...
	u8 edid_last_segment;
	u8 edid_current_segment;
	u8 audio_spkr_data[AUDIO_DATA_SIZE];
	struct workqueue_struct *workq;
	struct delayed_work adv7533_intr_work_id;
	struct delayed_work adv7533_hdcp_timer_work_id;
...
static struct adv7533_reg_cfg SPDIF_cfg[] = {
		{I2C_ADDR_CEC_DSI, 0x05, 0xC8, 0}, /* audio data and clock */
		{I2C_ADDR_MAIN, 0x01, 0x00, 0}, /* 48k */
		{I2C_ADDR_MAIN, 0x02, 0x18, 0}, /* 48k */
		{I2C_ADDR_MAIN, 0x03, 0x00, 0}, /* 48k */
		{I2C_ADDR_MAIN, 0x0A, 0x51, 0}, /* select spdif */
		{I2C_ADDR_MAIN, SPDIF_REG, SPDIF_ENABLED_VAL, 0}, /* spdif enable */
		{I2C_ADDR_MAIN, 0x0C, 0xB8, 0}, /* disable i2s */
};
...
static int adv7533_start_stop_hdcp(struct adv7533 *pdata, bool start);
int adv7533_sysfs_init(struct device *dev);
void adv7533_sysfs_remove(struct device *dev);
void adv7533_check_hdcp_timer_callback( unsigned long data );
DEFINE_TIMER(check_HDCP_timer, adv7533_check_hdcp_timer_callback, 0, 0);
...
static int adv7533_write(struct adv7533 *pdata, u8 offset, u8 reg, u8 val)
{
	u8 addr = 0;
	int ret = 0;
	pr_debug("%s: offset: %x  reg: %x  val: %x\n", __func__, offset, reg, val);
		...
		u8 reg, char *buf, u32 size)
{
	...
	pr_debug("%s: pdata: %p, offset: %x  reg: %x  size: %x\n", __func__, pdata, offset, reg, size);
	...
	struct adv7533 *pdata)
{
	...
	pdata->audio_enabled = of_property_read_bool(np, "adi,enable-audio");
	pdata->hdcp_enabled = of_property_read_bool(np, "adi,enable-hdcp");
...
static int adv7533_start_hdcp(struct adv7533 *pdata)
{
	u8 ret = -EINVAL;

	if (pdata->hdcp_enabled && pdata->hdcp_on && pdata->is_connected){
		adv7533_start_stop_hdcp(pdata, false);
		ret = adv7533_start_stop_hdcp(pdata, true);
	}
	return ret;
}
...
void adv7533_check_hdcp_timer_callback(unsigned long data)
{
	struct adv7533 *pdata = (struct adv7533 *) data;

	pr_debug("%s: HDCP nanny \n", __func__);
	queue_delayed_work(pdata->workq, &pdata->adv7533_hdcp_timer_work_id, 0);
}

void adv7533_hdcp_timer_work(struct work_struct *work)
{
	...
	pdata = container_of(dw, struct adv7533, adv7533_hdcp_timer_work_id);
	...
	/* we need to confirm we are still encrypted, if not start over */
	adv7533_read(pdata, I2C_ADDR_MAIN, HDCP_ENC_ON_REG, &reg_val, 1);
	if (!(reg_val & HDCP_ENC_ON_BIT)) {
		/* we are no longer encrypting so start over */
		adv7533_start_hdcp(pdata);
	}
	mod_timer(&check_HDCP_timer,
			jiffies + msecs_to_jiffies( HDCP_CHECK_DELAY_MS));
}
		...
		u8 auth_status)
{
	...
	if (auth_status & HDCP_AUTHENTICATED_INT_EN) {
		/* we are golden so lets kick off the nanny */
		pr_debug("%s: HDCP is now AUTHENTICATED.\n", __func__);
		mod_timer(&check_HDCP_timer,
				jiffies + msecs_to_jiffies( HDCP_CHECK_DELAY_MS));
	}

	if (hdcp_status & HDCP_ERROR_INT_BIT) {
		/* We got an error, the chip handles the error, we are just logging it for debug */
		adv7533_read(pdata, I2C_ADDR_MAIN, HDCP_ERR_STATE_REG, reg_val, 1);
		pr_err("%s: Got an error: %X \n", __func__, reg_val[0]);
	}

	if (hdcp_status & BKSV_FLAG_INT_BIT) {
		/* we are not going to do anything here.
		   We should check some lists but this is just
		   to get past the compliance testing
		 */
		pr_debug("%s: HDCP: Got BKSV int\n", __func__);
		/* read the BKSVs */
		adv7533_read(pdata, I2C_ADDR_MAIN, 0xBF, reg_val, sizeof(reg_val));

		for (i = 0; i < sizeof(reg_val); i++) {
			pr_debug(" %X", reg_val[i]);
		}
	}
}
...
static int adv7533_edid_read_init(struct adv7533 *pdata)
{
    ...
    memset(pdata->edid_buf, 0, sizeof(pdata->edid_buf));

	/* initiate edid read in adv7533 */
	adv7533_write(pdata, I2C_ADDR_MAIN, 0x41, 0x10);
    //TJP, instead of setting the reread bit, we can support more than
    // two edid blocks, therefore we will set the segment to zero instead.
    // This will kick off the read but in the right segment
    adv7533_write(pdata, I2C_ADDR_MAIN, ADV7533_REG_EDID_SEGMENT, 0);
...
static void *adv7533_handle_hpd_intr(struct adv7533 *pdata)
{
	...
	ret = adv7533_read(pdata, I2C_ADDR_MAIN, 0x42, &hpd_state, 1);
	if (ret)
		goto end;

	connected = (hpd_state & BIT(6));
	...
	if (connected) {
		...
		adv7533_start_stop_hdcp(pdata, false);
		pdata->is_connected = false;
		adv7533_notify_clients(&pdata->dev_info, MSM_DBA_CB_HPD_DISCONNECT);
	}
	ret = connected ? 1 : 0;
end:
	return ERR_PTR(ret);
...
static int adv7533_enable_interrupts(struct adv7533 *pdata, int interrupts)
{
	...
	if (interrupts & CFG_HDCP_INTERRUPTS)
		reg_val |= HDCP_AUTHENTICATED_INT_EN;
...
static void adv7533_intr_work(struct work_struct *work)
{
	...
	ret = adv7533_read(pdata, I2C_ADDR_MAIN, 0x96, &int_status, 1);
	if(ret) goto reset;
	/* Read the next set of ints */
	ret = adv7533_read(pdata, I2C_ADDR_MAIN, 0x97, &hdcp_cec_status, 1);
	if(ret) goto reset;
	...
	if (int_status & (BIT(6) | BIT(7))) {
		...
		connected = ret;
		pdata->edid_current_segment = 0;
		pdata->edid_last_segment = 0;
	}

	/* EDID ready for read */
	if ((int_status & BIT(2)) && pdata->is_power_on) {
		pr_debug("%s: EDID READY: last seg = %d Segment: %d\n", __func__,
				pdata->edid_last_segment,
				pdata->edid_current_segment);

		/* some safety checks, see if edid_buf is large enough to hold next segment */
		if (((pdata->edid_current_segment + 1) * EDID_SEG_SIZE)
				> sizeof(pdata->edid_buf))
				{
			pr_err(
					"%s: edid read buffer too small!!!! size needed = %d, size of buffer = %lu\n",
					__func__, (pdata->edid_current_segment + 1) * EDID_SEG_SIZE,
					sizeof(pdata->edid_buf));
		} else {
			ret = adv7533_read_edid(pdata, EDID_SEG_SIZE,
					pdata->edid_buf
							+ (pdata->edid_current_segment * EDID_SEG_SIZE));

			if (ret) {
				pr_err("%s: edid read failed\n", __func__);
			} else if (pdata->edid_current_segment == 0) {
				/* We need to read byte 0x7E to see how many blocks there are in total.
				 We already have block 0 and 1(seg 0), so check if there are more than
				 one additional block.
				 */
				pr_debug("%s: edid_buf[0x7E] = %d\n", __func__,
						pdata->edid_buf[0x7e]);
				if (pdata->edid_buf[0x7E] > 1) {
					pdata->edid_current_segment = 1; /* we want segments 1 and 2 */
					pdata->edid_last_segment = pdata->edid_buf[0x7E]
							* EDID_BLOCK_SIZE / EDID_SEG_SIZE; /* This will truncate to give the correct integer */
					/* Writing to the segment address in the 7535 will kickoff a read.*/
					adv7533_write(pdata, I2C_ADDR_MAIN,
							ADV7533_REG_EDID_SEGMENT,
							pdata->edid_current_segment);
					/* bail out because we will get another interrupt */
					goto cont_edid;
				}
			} else if (pdata->edid_current_segment < pdata->edid_last_segment) {
				pdata->edid_current_segment++;
				pr_debug("%s: edid_seg = %d\n", __func__, pdata->edid_current_segment);
				adv7533_write(pdata, I2C_ADDR_MAIN, ADV7533_REG_EDID_SEGMENT,
						pdata->edid_current_segment);
				/* bail out because we will get another interrupt */
				goto cont_edid;
			}
		...
		pdata->edid_current_segment = 0;
		pdata->edid_last_segment = 0;
		pdata->is_connected = true;
		adv7533_notify_clients(&pdata->dev_info, MSM_DBA_CB_HPD_CONNECT);
		adv7533_start_hdcp(pdata);
	}
...
cont_edid:
	...
	if (pdata->hdcp_on && pdata->hdcp_enabled) {
		adv7533_handle_hdcp_intr(pdata, hdcp_cec_status, int_status);
	}
...
static int adv7533_start_stop_hdcp(struct adv7533 *pdata, bool start)
{
	...
	ret = adv7533_read(pdata, I2C_ADDR_MAIN, HDCP_HDMI_DVI_REG, &reg_val, 1);
	if (ret)
		goto end;

	if (start) {
		reg_val |= HDCP_DESIRED_BIT;
	} else {
		reg_val &= ~HDCP_DESIRED_BIT;
		del_timer(&check_HDCP_timer);
	}

	ret = adv7533_write(pdata, I2C_ADDR_MAIN, HDCP_HDMI_DVI_REG, reg_val);
end:
	return ret;
}


static int _adv7533_hdcp_enable(struct adv7533 *pdata, bool hdcp_on,
		bool enc_on, u32 flags)
{
	int ret = -EINVAL;

	pdata->hdcp_on = hdcp_on;
	pr_debug("%s: HDCP on: %d  connected: %d enabled: %d\n", __func__, hdcp_on,
			pdata->is_connected, pdata->hdcp_enabled);
	if ((ret = adv7533_start_stop_hdcp(pdata, false)))
		goto end;

	if (pdata->hdcp_on && pdata->hdcp_enabled && pdata->is_connected)
			{
		if ((ret = adv7533_enable_interrupts(pdata, CFG_HDCP_INTERRUPTS)))
			goto end;
		if ((ret = adv7533_start_stop_hdcp(pdata, true)))
			goto end;
	} else {
		if ((ret = adv7533_disable_interrupts(pdata, CFG_HDCP_INTERRUPTS)))
			goto end;
		ret = adv7533_start_stop_hdcp(pdata, false);
	}
...
static int adv7533_hdcp_enable(void *client, bool hdcp_on, bool enc_on,
		u32 flags)
{
	int ret;
	...
	ret = _adv7533_hdcp_enable(pdata, hdcp_on, enc_on, flags);
...
void set_vid_infoframe(struct msm_dba_video_cfg* cfg, struct adv7533* pdata)
{
	u8 reg_val = 0;

	pr_debug("%s: \n", __func__);
	/* set AVI_UPDATE */
	adv7533_read(pdata, I2C_ADDR_MAIN, ADV7533_REG_INFOFRAME_UPDATE, &reg_val, 1);
	reg_val &= ~(BIT(3) | BIT(2)); /* clear out what we are going to be setting */
	reg_val |= BIT(6);

	/* The 7535 has a bit to specify the frame rate is low, meaning
	 below 50Hz. If we are running say, 30Hz or 24Hz, we need to set
	 the value.
	 The two bits are in reg 0x4A too, so lets set them now.
	 We only support 1080p30 and 1080p24 so we can just filter on the vic
	 */
	if (cfg->vic == HDMI_VFRMT_1920x1080p30_16_9) {
		reg_val |= BIT(3) | BIT(2);
	} else if (cfg->vic == HDMI_VFRMT_1920x1080p24_16_9) {
		reg_val |= BIT(3);
	}

	pr_debug("%s: vic = %d reg = %x\n", __func__, cfg->vic, reg_val);
	adv7533_write(pdata, I2C_ADDR_MAIN, ADV7533_REG_INFOFRAME_UPDATE, reg_val);
	...
	if (cfg->h_active * 3 - cfg->v_active * 4) {
		...
		/* 4:3 aspect ratio */
		adv7533_write(pdata, I2C_ADDR_MAIN, 0x17, 0x60);
		adv7533_write(pdata, I2C_ADDR_MAIN, 0x56, 0x18);
	}

	/* clear AVI_UPDATE, this should trigger sending of infoframe? waiting for confirmation from ADV */
	adv7533_read( pdata, I2C_ADDR_MAIN, ADV7533_REG_INFOFRAME_UPDATE, &reg_val, 1 );
	reg_val &= ~BIT(6);
	adv7533_write(pdata, I2C_ADDR_MAIN, ADV7533_REG_INFOFRAME_UPDATE, reg_val);
}
	...
	struct msm_dba_video_cfg *cfg, u32 flags)
{
	...
	/* there is nothing to do if we are turning off video. */
	if(!on)
		return ret;
	...
	/* hdmi/dvi mode */
	adv7533_read( pdata, I2C_ADDR_MAIN, HDCP_HDMI_DVI_REG, &reg_val, 1);
	reg_val &= ~HDMI_DVI_SEL_BIT;
	reg_val |= cfg->hdmi_mode ?  HDMI_DVI_SEL_BIT : 0;

	adv7533_write(pdata, I2C_ADDR_MAIN, HDCP_HDMI_DVI_REG, reg_val);

	/* set scan info for AVI Infoframe*/
	set_vid_infoframe(cfg, pdata);

	adv7533_write_array(pdata, adv7533_video_en, sizeof(adv7533_video_en));

	/* need to reconfigure audio */
	if (pdata->audio_enabled) {
		adv7533_write_array(pdata, SPDIF_cfg, sizeof(SPDIF_cfg));
	}
...
static int adv7533_audio_enable(struct adv7533 *pdata, bool enable)
{
	int ret = 0;
	pr_debug("%s: enabled: %d\n", __func__, enable);

	if (enable) {
		ret = adv7533_write_array(pdata, SPDIF_cfg, sizeof(SPDIF_cfg)); /* write whole array just in case */
	} else {
		/* just need to disable spdif */
		ret = adv7533_write(pdata, I2C_ADDR_MAIN, SPDIF_REG,
				SPDIF_DISABLED_VAL);
	}
	return ret;
}
	 ...
	 const struct i2c_device_id *id)
{
	...
	if (ret) {
		pr_err("%s: msm dba sysfs init failed\n", __func__);
		goto err_dba_helper;
	}

	ret = adv7533_sysfs_init(&client->dev);
	if (ret) {
		pr_err("%s: adv7533 sysfs init failed\n", __func__);
		goto err_dba_helper;
	}
	...
	if (pdata->audio_enabled) {
		...
		if (adv7533_write_array(pdata, SPDIF_cfg, sizeof(SPDIF_cfg)))
			goto end;
	}

	check_HDCP_timer.data = (unsigned long)pdata;

	INIT_DELAYED_WORK(&pdata->adv7533_intr_work_id, adv7533_intr_work);
	INIT_DELAYED_WORK(&pdata->adv7533_hdcp_timer_work_id, adv7533_hdcp_timer_work);
...
err_workqueue:
	...
	adv7533_sysfs_remove(&client->dev);
err_dba_helper:
...
static int adv7533_suspend(struct device *dev)
{
	int rc = -EINVAL;
	struct msm_dba_device_info *mdidev;
	struct adv7533 *pdata;
	struct dss_module_power *power_data = NULL;

	mdidev = dev_get_drvdata(dev);
	pdata = container_of(mdidev, struct adv7533, dev_info);
	if (!pdata)
		goto end;

	/* adv7533 power controlled by LPM; not an error case */
	power_data = &pdata->power_data;
	if (!power_data || !power_data->num_vreg) {
		return 0;
	}
...
static int adv7533_resume(struct device *dev)
{
	int rc = -EINVAL;
	struct msm_dba_device_info *mdidev;
	struct adv7533 *pdata;
		...
		char *buf)
{
	...
	pdata = container_of(dev_get_drvdata(dev), struct adv7533, dev_info);
	if (!pdata)
		goto end;

	bytes = snprintf(buf, PAGE_SIZE, "%d\n", pdata->hdcp_enabled);
		...
		size_t count)
{
	...
	mutex_lock(&pdata->ops_mutex);
	pdata->hdcp_enabled = !!enable;
	_adv7533_hdcp_enable(pdata, pdata->hdcp_on, true, 0);
	mutex_unlock(&pdata->ops_mutex);
...
static ssize_t audio_en_rda_attr(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	struct adv7533 *pdata;
...
static ssize_t audio_en_wta_attr(struct device *dev,
		struct device_attribute *attr,
		const char *buf,
		size_t count)
{
	...
	mutex_lock(&pdata->ops_mutex);
	pdata->audio_enabled = !!enable;
	adv7533_audio_enable(pdata, enable);
	mutex_unlock(&pdata->ops_mutex);
...
static DEVICE_ATTR(hdcp_en, S_IRUGO | S_IWUSR, hdcp_en_rda_attr, hdcp_en_wta_attr);
static DEVICE_ATTR(audio_en, S_IRUGO | S_IWUSR, audio_en_rda_attr, audio_en_wta_attr);

static struct attribute *adv7533_sysfs_attrs[] = {
		&dev_attr_hdcp_en.attr,
		&dev_attr_audio_en.attr,
		NULL,
};

static struct attribute_group adv7533_sysfs_attr_grp = {
		.attrs = adv7533_sysfs_attrs,
};
...
int adv7533_sysfs_init(struct device *dev)
{
	...
	rc = sysfs_create_group(&dev->kobj, &adv7533_sysfs_attr_grp);
	if (rc)
...
void adv7533_sysfs_remove(struct device *dev)
{
	...
	sysfs_remove_group(&dev->kobj, &adv7533_sysfs_attr_grp);
}


static SIMPLE_DEV_PM_OPS(adv7533_pm_ops, adv7533_suspend, adv7533_resume);
