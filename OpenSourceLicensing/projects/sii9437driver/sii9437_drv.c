...
#define SII9437_CHANNEL_STATUS_SIZE		9
...
#define SII_FREQ_128_KHZ		0x2B		/* "128 KHz " */
	...
	uint8_t *p_data, uint16_t length)
{
	...
	if (p_obj->b_standby_mode) {
		SII_LOG1A("", NULL, ("Standby Mode: Skipping eARC EDID set\n"));
	} else if ((offset + length) <= SII_EDID_BLOCK_SIZE) {
		...
		SII_LOG1A("", NULL, ("Error: Invalid Args for EDID Set\n"));
	...
	uint8_t *p_data, uint16_t length)
{
	...
	if ((offset + length) <= SII_EDID_BLOCK_SIZE) {
		SII_MEMCPY(p_data, p_obj->caps_ds + offset, length);
	} else {
		SII_LOG1A("", NULL, ("Error: Invalid Args for EDID Get\n"));
		status = SII_RETURN_VALUE__INVALID_ARG_ERR;
	}
...
static void s_sm_state_update(struct obj_t *p_obj, uint8_t *int_regs)
{
	...
	if ((int_regs[REG_INTR_0] & BIT_MASK__INTR0_MASK__STATE_IDLE1) ||
		...
		p_obj->sm_state = (enum sii9437_sm_state)sm_state; //lb1032816: Compiler warning fix
		...
		bool_t b_extra_bytes)
{
	if (b_extra_bytes) {
		SII_LOG1A("", p_obj,
			("CS: %02x.%02x.%02x.%02x.%02x.%02x.%02x.%02x.%02x",
...
static bool_t s_is_hbra(struct obj_t *p_obj)
{
	return ((p_obj->channel_status.data[3] & 0x0F) == SII_FREQ_4x192_KHZ) ?
		TRUE : FALSE;
}
...
static void s_update_samp_freq(struct obj_t *p_obj)
{
	...
	frame_rate	= ((p_obj->channel_status.data[3] & 0x0F) |
			((p_obj->channel_status.data[3] & 0xC0) >> 2));
	samp_freq	= s_get_freq(frame_rate);

	SII_LOG2A("", p_obj, ("frame_rate:%u samp_freq:%u",
	...
	SII_LOG2A("", p_obj, ("frame_rate:0x%x samp_freq:%u",
			frame_rate, samp_freq));

	/* Storing bits 24,25,26,27 into ch status byte 3 */
	p_obj->channel_status.data[3] = ((p_obj->channel_status.data[3] &
						!0x0F)	| (frame_rate & 0x0F));
	/* Storing bits 30,31 into ch status byte 3 */
	p_obj->channel_status.data[3] |= ((p_obj->channel_status.data[3] &
					!0xC0)	| ((frame_rate & 0x30) << 2));
...
static void s_audio_update(struct obj_t *p_obj)
{
	...
	SII_LOG1A("", p_obj,
		("Sampling Freq : %s\n",
		s_getting_freq_name(p_obj->channel_status.data[3] & 0x0F)));
	...
	} else if (s_is_non_lpcm(p_obj)) {
		mode = SII9437_EXTRACTION_MODE__SPDIF2;
		SII_LOG1A("", p_obj,
			("Compressed Audio: Force extr mode to SPDIF2\n"));
	} else if (!multi_ch || (multi_ch && s_get_layout(p_obj) ==
