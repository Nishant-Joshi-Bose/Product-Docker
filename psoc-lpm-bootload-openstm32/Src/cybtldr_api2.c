    ...
    unsigned char appId, CyBtldr_CommunicationsData* comm, CyBtldr_ProgressUpdate* update)
{
    ...
    if (CYRET_SUCCESS == err)
    {
        ...
        if (CYRET_SUCCESS == err)
            err = CyBtldr_ParseHeader(lineLen, (unsigned char*)line, &siliconId, &siliconRev, &chksumtype);
        ...
        if (CYRET_SUCCESS == err)
        {
            while (CYRET_SUCCESS == err)
            {
                ...
                err = CyBtldr_ReadLine(&lineLen, line);
                if (CYRET_SUCCESS == err)
                    err = CyBtldr_ParseRowData(lineLen, (unsigned char*)line, &arrayId, &rowNum, buffer, &bufSize, &checksum);
                if (CYRET_SUCCESS == err)
                {
                    switch (action)
                    {
