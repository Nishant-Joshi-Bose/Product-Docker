...
#from BSerialLogger import SerialLoggingUtil
...
class Test_lightbar():
    ...
    def test_deleteActiveAnimation(self,get_FrontdoorInstance):
        ...
        if "value" in currentData:
            ...
            if "error" in animation_keys:
                assert "description" in (animation_keys["error"]).keys(),"Animation is all played successfully"
    ...
    def test_playInValidAnimationScenarios(self,get_FrontdoorInstance):
        ...
        with open("ei_Bose_LightBar_Test_Scenarios.csv") as ScenarioFile:
            ...
            for row in reader:
                sleep(2)
                #['CONNECTED', '123', 'TRUE', 'empty', 'animation not supported']
    ...
    def test_playValidAnimationScenarios(self,get_FrontdoorInstance):
        ...
        print("Playing positve tests for lightbar animation")
        ...
        logger.debug("List of supported animations:\n")
                ...
                for b in list_repeat:
                    ...
                    if "error" in animation_state1.keys():
                        ...
                        if list_failScenarios:
                            logger.debug("Some scenarios are failed.They are :--\n %s" % list_failScenarios)
                            assert "False","Failure found"
                    else:
                        logger.debug("Scenarios passed")
                    break
        logger.debug("ALL POSITIVE SCENARIOS PASSED")
        logger.debug("SerialDetails %s" % SerialDetails)
