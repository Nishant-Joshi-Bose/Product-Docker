...
def test_audio_treble_state(request, device_state, wifi_config, device_id):
    """
    ...
    # check if api exists
    eddie_helper.check_if_end_point_exists(frontdoor, eddie_helper.AUDIO_TREBLE_API)

    # verify get /audio/treble api
    response = eddie_helper.verify_audio_get_api(frontdoor, eddie_helper.AUDIO_TREBLE_API)
    ...
    eddie_helper.verify_audio_put_api(frontdoor, eddie_helper.AUDIO_TREBLE_API,
                                      initial_audio_data)
...
def test_audio_treble_source(request, front_door_queue, device_source, device_id):
    """
    ...
    # check if api exists
    eddie_helper.check_if_end_point_exists(front_door_queue, eddie_helper.AUDIO_TREBLE_API)

    # verify get /audio/treble api
    response = eddie_helper.verify_audio_get_api(front_door_queue, eddie_helper.AUDIO_TREBLE_API)

    # capture data for put request verification
    initial_audio_data = response["body"]

    # verify put /audio/treble api and notification response
    eddie_helper.verify_audio_put_api(front_door_queue, eddie_helper.AUDIO_TREBLE_API,
                                      initial_audio_data)
...
def test_audio_treble_playing_amazon(front_door_queue):
    """
    Test for verifying get and put and notify request of audio treble api
...
@pytest.mark.usefixtures("request", "wifi_config", "ip_address_wlan", "device_id")
@pytest.mark.parametrize("device_state", ["OFF", "ON", "SETUP"])
def test_audio_treble_invalid_value(request, device_state, wifi_config, device_id):
    """
...
@pytest.mark.usefixtures("request", "wifi_config", "ip_address_wlan", "device_id")
@pytest.mark.parametrize("device_state", ["OFF", "ON", "SETUP"])
def test_audio_treble_keyerror(request, device_state, wifi_config, device_id):
    """
    ...
    eddie_helper.verify_audio_keyerror(frontdoor, data_invalid_key, eddie_helper.AUDIO_TREBLE_API)
    ...
    eddie_helper.verify_audio_keyerror(frontdoor, data_invalid, eddie_helper.AUDIO_TREBLE_API)
