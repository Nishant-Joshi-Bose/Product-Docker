...
@pytest.mark.usefixtures('driver', 'device_id', 'device_ip')
def test_diagnostics_mfgdata(driver, device_id, device_ip, request):
    ...
    try:
        ...
        diagnostics_page = DiagnosticsPage(driver, logger=logger, deviceid=device_id)
        diagnostics_page.system_reboot(device_id, device_ip)
