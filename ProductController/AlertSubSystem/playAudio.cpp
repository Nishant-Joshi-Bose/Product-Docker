		...
		if (read(inHandle, &head, sizeof(FILE_head)) == sizeof(FILE_head))
		{
			...
			while (read(inHandle, &head, sizeof(CHUNK_head)) == sizeof(CHUNK_head))
			{
				...
				else if (compareID(&Data[0], &head.ID[0]))
				{
					...
					if ((unsigned)read(inHandle, WavePtr, head.Length) != head.Length)
					{
						free(WavePtr);
						break;
					}
...
int playAlert(char *wavefile)
{
	...
	if (wavefile == NULL)
		ret = waveLoad ("default.wave");
	else
		ret = waveLoad (wavefile);
	...
	if (!ret)
	{
		...
		else
		{
			...
			if ((err = snd_pcm_set_params(PlaybackHandle, fmt, SND_PCM_ACCESS_RW_INTERLEAVED, WaveChannels, WaveRate, 1, 500000)) < 0)
			...
			// Play the waveform
				...
				do
				{
					play_audio();
					sleep(1);	
			err = snd_pcm_set_params(PlaybackHandle, fmt, SND_PCM_ACCESS_RW_INTERLEAVED, WaveChannels, WaveRate, 1, 500000);
				}while(play); //need to replace with a signal interrupt
			}
...
//need to refactor the code to encapsulate in cpp style
int stopAlert ()
{
	printf("play = %d\n",play);
	play = 0;
