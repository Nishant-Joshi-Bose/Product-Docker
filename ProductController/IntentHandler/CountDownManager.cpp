...
static std::unordered_map < ProductApp::KeyActionMode_t, CountDownInfo> m_countdownIntentInfoMap =
{
    ...
    {( std::make_pair( ProductApp::Action::FORCED_SOFTWARE_UPDATE_COUNTDOWN, 0 ) ), {ButtonEventName::FORCED_SOFTWARE_UPDATE, 10}}
};
...
bool CountDownManager::Handle( KeyHandlerUtil::ActionType_t& intent )
{
    ...
    case( uint16_t ) Action::FORCED_SOFTWARE_UPDATE_CANCEL:
    {
    ...
    case( uint16_t ) Action::FORCED_SOFTWARE_UPDATE_COUNTDOWN:
    {
